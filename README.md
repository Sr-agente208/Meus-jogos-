...
No menu de pausa, selecione **Automático**, **PC / Teclado** ou **Mobile / Toque** para definir os controles. O progresso e as opções do protótipo são salvos localmente no navegador. O prólogo conduz o jogador pela cidade de Verade: espelho `0x7` → três Ecos Azuis → Núcleo Azul → Protocolo Espelho. Consulte [`docs/MECANICAS.md`](docs/MECANICAS.md) para detalhes da versão implementada.
